...
set -e
CASSANDRA_CONF_DIR=/opt/apache-cassandra-3.0.12/conf
CASSANDRA_CFG=$CASSANDRA_CONF_DIR/cassandra.yaml

if [ -n "$CASSANDRA_SEED_POD" ]; then
  echo "Generating a CASSANDRA_SEEDS based on CASSANDRA_SEED_POD ('${CASSANDRA_SEED_POD}')"
  CASSANDRA_SEEDS="${CASSANDRA_SEED_POD}.${POD_NAMESPACE}.svc.cluster.local"
fi

# we are doing StatefulSet or just setting our seeds
if [ -z "$CASSANDRA_SEEDS" ]; then
  echo "Generating a CASSANDRA_SEEDS based on the hostname"
  HOSTNAME=$(hostname -f)
